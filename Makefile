...
coverage:
	coverage run --source mri_tools setup.py test
	coverage report -m
	coverage html
	@echo "To view results type: htmlcov/index.html &"

docs:
	rm -f docs/mri_tools.rst
	rm -f docs/modules.rst
	sphinx-apidoc -f -o docs/ mri_tools
	$(MAKE) -C docs clean
	$(MAKE) -C docs html
...
install: dist
	pip install --upgrade --no-deps --force-reinstall dist/mri_tools-*.tar.gz

uninstall: 
	pip uninstall -y mri_tools
